      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          double throttle_value  = speed_pid.Thtottle(steer_value);

          if (pid.is_twiddle){
            pid.twiddle();
            if (pid.step == 0){ Restart(ws);}
            if (fabs(cte) > 2.2){
              pid.step = pid.end_step + 1;
              pid.total_error = 99999999.9;
              Restart(ws);}
            if (pid.finished) { ws.close();}
          }
